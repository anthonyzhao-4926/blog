...
965 行、10 个分类，不用全背。先掌握「必会」这六类，剩下的用到再回来查：

| 分类                | 程度   | 主要内容                             |
| ----------------- | ---- | -------------------------------- |
| [目录处理命令](#目录处理命令) | 必会   | `ls` `cd` `mkdir` `cp` `mv` `rm` |
| [文件处理命令](#文件处理命令) | 必会   | 查看、创建、链接文件                       |
| [权限管理命令](#权限管理命令) | 必会   | `chmod` `chown`，权限位怎么算           |
| [文件搜索命令](#文件搜索命令) | 必会   | `find` `locate`                  |
| [帮助命令](#帮助命令)     | 必会   | `man`，忘了参数就来这                    |
| [压缩命令](#压缩命令)     | 必会   | `tar` `zip`，打包与解包                |
| [用户管理命令](#用户管理命令) | 用到再查 | 多用户与权限体系才用得上                     |
| [网络命令](#网络命令)     | 用到再查 | 排查网络时查                           |
| [关机重启命令](#关机重启命令) | 用到再查 |                                  |
| [系统运行级别](#系统运行级别) | 用到再查 |                                  |
...
**下一篇**：[文本编辑器 vim](03-文本编辑器vim.md)——服务器上改配置文件绕不开它。
