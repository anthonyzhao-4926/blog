...
| `effort`                   | 1        | 默认继承自会话，详见[effort](09-effort、设置优先级与模型.md#effort)             |                                                              |
...
2. 读取 `assets/template.md`，按占位符填空后整份交给用户（保留标题和三个章节，不要改结构）：
...
4. 仅当用户追问语气或字数时，再读 `references/style.md` 并按其约束改一版。
...
**下一篇**：[Subagent](07-Subagent.md)——把支线任务挪出主上下文。
