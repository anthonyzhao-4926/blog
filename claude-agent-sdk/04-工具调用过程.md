...
> **读完这篇你能**：看懂工具调用与工具结果的消息形态，理解它们为什么都挂在 user 角色上。
> **前置**：[流式输出](02-流式输出.md)。约 15 分钟。
...
顺带提一句，`user` → `message 块` 这一对构成一个来回，称作一个 **turn**——它的边界单独写在了[Turn](08-turn.md)。

---

**下一篇**：[agent 进度跟踪](05-agent%20进度跟踪.md)——工具里最特殊的一个是 Agent，它会引入并发。
