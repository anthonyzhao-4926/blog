...
| 我想…… | 看这篇 |
| --- | --- |
| 知道 MCP 有哪几种传输、该选哪个 | [Transport 对照表](参考/11-Transport对照表.md) |
| 搞清泛型 handler 到底帮我做了什么 | [工具签名](参考/12-工具签名.md) |
...
| 查错误码含义 | [错误码](参考/13-错误码.md) |

### 参考卡片

以下不是读物，是查到才用的字典，不用顺序读：

- [Transport 对照表](参考/11-Transport对照表.md)——九种 transport 各自什么场景，一张表查完
- [工具签名](参考/12-工具签名.md)——泛型 handler 的签名、SDK 自动做了哪些事
- [错误码](参考/13-错误码.md)——JSON-RPC 错误码速查
- [go:embed 速查](参考/14-goembed速查.md)——把模板、图片打进二进制
- [Content](参考/15-Content.md)——消息里「一段内容」有哪些类型
- [MIME 类型](参考/16-MIME类型.md)——MIME 是什么、MCP 里常用哪些取值
- [日志系统字段与查询语法](参考/17-日志系统字段与查询语法.md)——主线案例的测试素材
- [分析日志 Prompt 模板](参考/18-分析日志Prompt模板.md)——主线案例的提示词正文
