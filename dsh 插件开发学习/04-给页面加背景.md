...
>
> **前置**：[改掉 DSH 的默认装配](03-改掉DSH的默认装配.md)。约 25 分钟。

[上一篇](03-改掉DSH的默认装配.md)把 patch 讲完了，但它改的都是 DSH 自带的那几行。现在回到我们自己的插件——写一个真正能动 UI 的。实现如下效果：
...
**下一篇**：[effect 与插件生命周期](05-effect与生命周期.md)——把这篇里 `ctx.effect` 那几行为什么非写不可讲透。
