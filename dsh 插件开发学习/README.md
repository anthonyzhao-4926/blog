...
- **骨架**：任务型，对上 [`专栏写作规范.md`](../专栏写作规范.md)——开头三行（读完你能 / 前置 / 约 N 分钟），正文按「什么时候需要 → 改哪里 → 完整代码 / 代码解释 → 注意事项」，结尾以 `**下一篇**：` 开头链到下一篇。
...
- 01–06 已经写好：**06 现在是链路末篇**，暂时挂着终点标记；写 07 时把它换成指向 07 的「**下一篇**：」，终点标记继续往后传。同时把「一条主线」的有序列表补上 07，并把九级表里对应的行标上（已写）。
