...
1. [插件与 profile](01-插件与profile.md)——profile、bundle、插件三者的关系，建一个干净的 test profile
2. [第一个插件](02-第一个插件.md)——一个插件包最少要有哪些文件，怎么被 `dsh plugin add` 自动装配
3. [改掉 DSH 的默认装配](03-改掉DSH的默认装配.md)——用 patch 改内置行、插自己的行、停用不要的行
4. [给页面加背景](04-给页面加背景.md)——注册 HTTP 路由，往页面里注入 CSS
5. [effect 与插件生命周期](05-effect与生命周期.md)——插件从加载到卸载的四个阶段，什么必须交给 `ctx.effect`
6. [让插件可配置](06-让插件可配置.md)——`Config` + schema，配置的层叠与覆盖
7. [改代码不想重启](07-改代码不想重启.md)——patch 与源码的热重载，哪些改动必须重启
...
| 让插件能被自动装配（不用手改 profile） | [第一个插件](02-第一个插件.md) → `package.json` 一节 |
...
| 改一行代码／改一处 patch 不想重启 | [改代码不想重启](07-改代码不想重启.md) |
| 改了没生效，终端还什么都不说 | [改代码不想重启](07-改代码不想重启.md) → 「注意事项」 |
| 查 `patchReload` 有哪些取值、省略时算哪个 | [profile 与插件包结构](参考/profile与插件包结构.md) → 「patchReload 速查」 |
| 关掉一个内置插件，或改它的参数 | [改掉 DSH 的默认装配](03-改掉DSH的默认装配.md) |
| 不想用了，怎么停用而不是卸载插件 | [改掉 DSH 的默认装配](03-改掉DSH的默认装配.md) → 「停用插件」一节 |
...
撰写文档同时要完成代码，代码放置在 `/Users/anthonyzhao/anthonyzhao/blog/dsh_plugin`
...
| ------ | ------------------ | ---------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| 03（已写） | `03-改掉DSH的默认装配.md` | 内置 bash 直连本机不放心，或某个内置插件根本不想要 | patch 的 insert / id+字段 / disabled；bundles → profile patch → home patch → `--patch` 的叠加顺序；`!!js` 表达式；id 找不到与文件写错两种失败待遇 |
| 06（已写） | `06-让插件可配置.md`     | 换张背景图、调个透明度都要改代码重装           | `Config` + Schemastery schema；`cordis.yml` 的 `config` 块；配置层叠与 patch 覆盖；默认值放哪；非法配置为什么必须加载失败                            |
| 07（已写） | `07-改代码不想重启.md`    | 改一行就重启，迭代太慢；改了没生效还说不出为什么     | HMR 覆盖哪些改动、哪些必须重启；`patchReload` 的 `live` / `startup`；Fiber 状态机；用 `--dump-config` 排查装配                                 |
...
- **骨架**：对上 [`专栏写作规范.md`](../专栏写作规范.md)——开头三行（读完你能 / 前置 / 约 N 分钟），正文按「**目标** → 改哪里 / 第一步 → 完整代码 / 代码解释 → 注意事项」，结尾以 `**下一篇**：` 开头链到下一篇。规范里的任务型首节用「什么时候需要」，本栏统一改用「**目标**」：先说清这篇改完是什么样，不适用的人自己会走。
...
- 01–07 已经写好：**07 现在是链路末篇**，暂时挂着终点标记；写 08 时把它换成指向 08 的「**下一篇**：」，终点标记继续往后传。同时把上面「一条主线」的有序列表补上 08，并把九级表里对应的行标上（已写）。
