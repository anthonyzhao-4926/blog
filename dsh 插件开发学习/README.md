...
**能**：写出一个能被 `dsh plugin add` 自动装配的插件包；用 `ctx.inject` 拿到内部服务、注册 HTTP 路由；监听注入类事件往页面里塞样式；理解插件从加载到卸载的四个阶段，知道什么必须交给 `ctx.effect` 管理；把写死的参数抽成 `Config`，在 profile 里覆盖它；用 patch 关掉或改写 DSH 自带插件的装配，不动 `node_modules`。

**不能**：不覆盖 Cordis 的全部 API（那是 [`cordis_api.md`](../dsh/cordis_api.md) 的事）；01–06 不涉及 React 客户端组件与 slot 体系（需求走到「界面」时才会遇到）；不解释 DSH 内核为什么这么设计（那是 `docs/subsystems/` 的事）。
...
**04–06 共用一个案例**：做一个把本地图片设成 DSH 聊天背景的插件。04 是那个背景插件，05 把它用到的生命周期概念讲透，06 把它的参数挪进 `Config`。01–03 是地基（profile 是什么、插件最小长什么样、patch 怎么合成一棵树）。

**从 03 起每篇单独选例子**：03 讲 patch，用的是 DSH 自带的那几行（内置搜索、网页抓取）——那个需求本来就是「改别人装的插件」；再往后讲「换供应商」用 DSH 的搜索能力，讲「审计」用工具调用日志，讲「写工具」用工作区笔记搜索……哪个例子把这个需求讲得最清楚就用哪个。需求不适合背景插件的，就不拿它硬套——全栏共用同一个例子既不是目标，也不是要求。
...
3. [改掉 DSH 的默认装配](03-改掉DSH的默认装配.md)——`patch.yaml` 到底在干什么，怎么用它关掉内置插件、改内置参数
4. [给页面加背景](04-给页面加背景.md)——注册图片路由 + 注入样式，打通"让 UI 变样"的两条路
5. [effect 与插件生命周期](05-effect与生命周期.md)——四个阶段、fiber 是什么、为什么必须清理
6. [让插件可配置](06-让插件可配置.md)——把图片路径、透明度挪进 `Config`，改配置不用改代码
...
| `patch.yaml` 里的 `insert` 是什么意思 | [改掉 DSH 的默认装配](03-改掉DSH的默认装配.md) |
| 注册一个 HTTP 路由 | [给页面加背景](04-给页面加背景.md) |
| 往页面里注入 CSS 或脚本 | [给页面加背景](04-给页面加背景.md) |
| 插件卸载后路由还留着 / 报 duplicate route | [effect 与插件生命周期](05-effect与生命周期.md) |
| 让插件里某个值可配（换图、调透明度） | [让插件可配置](06-让插件可配置.md) |
| 配置写了不生效 / 想覆盖插件的默认值 | [让插件可配置](06-让插件可配置.md) → 「配置的层叠」一节 |
| 关掉一个内置插件，或改它的参数 | [改掉 DSH 的默认装配](03-改掉DSH的默认装配.md) |
| 不想用了，怎么停用而不是卸载插件 | [改掉 DSH 的默认装配](03-改掉DSH的默认装配.md) → 「怎么"停用"一个插件」一节 |
...
01–06 也是需求驱动的：04 的需求是「让页面变样」，05 是被它引出的「为什么必须清理」；03 是唯一的例外，它是被 02 引出的「`patch.yaml` 到底在干什么，我能不能也用它改 DSH 自己」，所以插在 02 后面。差别只在——04–06 恰好共用一个案例，03 和 07 起不共用。
...
#### 第一级：让 DSH 贴合我的环境（03，05–08）

「让插件贴合我的环境」这一级从 03 就开始了：03 教的是**改 DSH 自己的装配**，把它提前到 02 后面，是因为 `patch.yaml` 在 02 就已经出现过了，早解释早省事。

| 编号 | 文件 | 需求（什么场景下看这篇） | 讲清 DSH 的什么 |
| --- | --- | --- | --- |
| 03 | `03-改掉DSH的默认装配.md` | 内置 bash 直连本机不放心，或某个内置插件根本不想要 | patch 的 insert / id+字段 / disabled；bundles → profile patch → home patch → `--patch` 的叠加顺序；`!!js` 表达式；id 找不到与文件写错两种失败待遇 |
| 06 | `06-让插件可配置.md` | 换张背景图、调个透明度都要改代码重装 | `Config` + Schemastery schema；`cordis.yml` 的 `config` 块；配置层叠与 patch 覆盖；默认值放哪；非法配置为什么必须加载失败 |
| 07 | `07-改代码不想重启.md` | 改一行就重启，迭代太慢；改了没生效还说不出为什么 | HMR 覆盖哪些改动、哪些必须重启；`patchReload` 的 `live` / `startup`；Fiber 状态机；用 `--dump-config` 排查装配 |
...
- 01–06 已经写好：**06 现在是链路末篇**，暂时挂着终点标记；写 07 时把它换成指向 07 的「**下一篇**：」，终点标记继续往后传。
- patch 篇原本排在 06（那版规划里 06 是 HMR），后来提前到 **03**：`patch.yaml` 在 02 就出现了，拖到 06 才解释太晚。原来的 03/04/05 依次后移成 04/05/06（文件名、`order` 与所有交叉引用都已同步改过）。
