...
保存配置（或重启 DSH），背景变成 cat.jpg、透明度 0.5。什么都不写，就用插件自带的那张图、透明度 0.35。
...
自建 profile 默认 `patchReload: live`：改完 `cordis.patch.yml` **保存就生效**，不用重启启动命令（条件与边界见[改代码不想重启](07-改代码不想重启.md)）。
...
- **改 config 保存即生效。** 自建 profile 默认 `patchReload: live`，不用重启进程；怎么让改动即时生效、哪些改动必须重启，见[改代码不想重启](07-改代码不想重启.md)。

---

**下一篇**：[改代码不想重启](07-改代码不想重启.md)——改 patch 或改源码之后，不用重启进程就让它生效。
